...
const LicenceCertificationForm = ({ licenceCertifications, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    certification_title:'',
    company_name: '',
    time: '',
    company_name_logoUrl: null,
    certificatePdfUrl: null,
    skills: licenceCertifications?.skills || [],
  });
  ...
  useEffect(() => {
    if (licenceCertifications) {
      setFormData({
        ...
        company_name_logoUrl: licenceCertifications.company_name_logoUrl || null,
        certificatePdfUrl: licenceCertifications.certificatePdfUrl || null,
      });
    }
  }, [licenceCertifications]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (licenceCertifications) {
        await api.put(`/devs/licence-certification/${licenceCertifications._id}`, formData);
      } else {
        await api.post('/devs/licence-certification', formData);
      }
      onSubmit();
      onClose();
    } catch (error) {
      ...
      <div className="bg-[#2A2A32] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-lg">
        <div className="p-6 border-b border-gray-700">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-white">
              {licenceCertifications ? 'Edit' : 'Add'} Experience
            </h2>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-1 text-gray-400">certification_title</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="bg-[#1E1E24] border border-gray-700 rounded-lg p-4">
                {formData.company_name_logoUrl  ? (
                  <div className="relative inline-block">
                    <img 
                      src={formData.company_name_logoUrl } 
                      alt="company_name_logoUrl" 
                      className="w-24 h-24 rounded-lg object-cover"
                    />
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, company_name_logoUrl: null }))}
                  ...
                  <div className="flex flex-col items-center justify-center py-6">
                    <input
                        type="file"
                        name="company_name_logoUrl"  
                        id="company_name_logoUrl"
                        onChange={handleChange}
                        className="hidden"
                        accept="image/*"
                    />
                    <label
                      htmlFor="company_name_logoUrl"
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-400 text-sm">
                  {formData.certificatePdfUrl ? formData.certificatePdfUrl.name : 'No file chosen'}
                </span>
              </div>
              <input
                type="file"
                name="certificatePdfUrl"
                id="certificatePdfUrl"
                onChange={handleChange}
                className="hidden"
                accept=".pdf"
              />
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-gray-700">
            ...
            <button
              ...
              className="px-4 py-2 bg-[#FD356E] text-white rounded-md hover:bg-[#FF5F85] transition-colors"
            >
              {licenceCertifications ? 'Update' : 'Create'} Licence and Certification
            </button>
          </div>
        </form>
      </div>
...
export default LicenceCertificationForm;